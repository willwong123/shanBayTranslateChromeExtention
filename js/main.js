...
const APIURL = 'https://www.shanbay.com/api/v1/bdc/search/?version=2&word=';
let ex = 0;
let ey = 0;
let body = {};

const getWord = () => document.getSelection().toString()
  .replace(/^\s/g, '')
  .replace(/$\s/g, '');

const positionPop = (x, y) => {
  const pop = document.querySelector('#popupShanbay');
  const popH = pop.offsetHeight;
  const popW = pop.offsetWidth;
  const bodyH = body.offsetHeight > document.documentElement.clientHeight ?
    body.offsetHeight : document.documentElement.clientHeight;
  const bodyW = body.offsetWidth > document.documentElement.clientWidth ?
    body.offsetWidth : document.documentElement.clientWidth;
  // 情形一：单词在底部边界
...
const createPop = res => {
  while (!document.querySelector('#popupShanbay')) {
    ...
    if (res.status_code) {
      ...
      const defs = res.data.definitions.cn.map(def =>
        `<span class="definition">${def.pos} ${def.defn}</span>`);
      popInnerHtml = `<section>
        <p><b class="content">${res.data.content}</b></p>
        <p hidden><audio autoplay controls src="" data-src="${res.data.audio_addresses.us[0]}"/></p>
        <p>
          发音：[<span class="pronunciation">${res.data.pronunciations.us}</span>]
          <a href="javascript:;" class="sayWord">发音</a>
        </p>
        <p>
            翻译：
            ${defs} 
...
const translate = word => {
  const req = APIURL + word;
  fetch(req)
    .then(res => res.json())
    .then(res => createPop(res))
